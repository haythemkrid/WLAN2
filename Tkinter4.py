...
class APSelectionApp:
    def __init__(self, root, aps, tracker):
        self.root = root
        self.root.title("Select Access Points and Coordinates")
        self.aps = aps
        self.tracker = tracker  # Keep a reference to the APTracker instance
        self.picked_aps = {}
        ...
        self.plot_button = tk.Button(root, text="Plot APs", command=self.start_plot_refresh)
    ...
    def plot_aps(self):
        ...
        # List of colors to use for each circle
        colors = ['red', 'blue', 'green', 'purple', 'orange', 'brown', 'pink', 'cyan', 'yellow', 'lime']
        circles = []
        ...
        for i, ap in enumerate(self.aps):
            if ap['ESSID'] in self.picked_aps:
                ...
                # Use a color from the predefined list
                color = colors[i % len(colors)]  # Cycle through colors if there are more APs than colors
    ...
    def start_plot_refresh(self):
        # Fetch and plot data, then start the 5-second refresh cycle
        self.update_plot()

    def update_plot(self):
        # Fetch updated AP data
        self.tracker.fetch_APs()
        self.aps = self.tracker.show_APs()
...
def main():
    ...
    root = tk.Tk()
    app = APSelectionApp(root, aps, apt)  # Pass the tracker to the app
    root.mainloop()
